...
This R Notebook describes and demonstrates soil simulation modeling of SSURGO soil property data, leveraging sources of know uncertainty to better estimate the potential variability of soil properties, including both primary (e.g., texture, rock fragments) and derived using pedotransfer functions (e.g., PAWHC), and the propagation of uncertainty when generating derived data.
...
# NOTE: sim-functions.R was split into code/R/depth_simulation.R and
# code/R/property_simulation.R during the code/ reorganization; the old path
# (code/sim-functions.R, which never resolved even before the split) is fixed here.
source(here('code/R/depth_simulation.R'))
source(here('code/R/property_simulation.R'))
source(here('code/R/soil_infill_functions.R'))
```
...
# Step 1.0 Extract all ssurgo data in study area via SDA or load RSS data 
```{r}
# Choose which data source to run simulations

# Option 1: *** RUN SIMULATION WITH gSSURGO ***
# Extract mukeys from rasterized SSURGO data
s.rast <- rast(here('data/grids/ssurgo_utm.tif'))
rat <- cats(s.rast)[[1]]
...
# Option 2: *** RUN SIMULATION WITH RSS ***
# setup RSS data to run in place of SSURGO
load(here('data/rss-tab-data-raw.rda'))
...
ssurgo_data <- ssurgo_data %>%
  mutate(
    rfv_l = fraggt10_l + frag3to10_l,  # Total low estimate
    rfv_r = fraggt10_r + frag3to10_r,  # Total representative estimate
    rfv_h = fraggt10_h + frag3to10_h   # Total high estimate
  )

ssurgo_data <- ssurgo_data %>% dplyr::select(id, mukey, cokey, compname, comppct_l, comppct_r, comppct_h, chkey, hzname, hzdept_l, hzdept_r, hzdept_h, hzdepb_l, hzdepb_r, hzdepb_h, hzthk_l, hzthk_r, hzthk_h, sandtotal_l, sandtotal_r, sandtotal_h, silttotal_l, silttotal_r, silttotal_h, claytotal_l, claytotal_r, claytotal_h, dbovendry_l, dbovendry_r, dbovendry_h, wthirdbar_l, wthirdbar_r, wthirdbar_h, wfifteenbar_l, wfifteenbar_r, wfifteenbar_h, rfv_l, rfv_r, rfv_h)
...
```{r}
ssurgo_soil_sim_df <- simulate_soil_properties(ssurgo_data@horizons)
```


# Step 2: Reshape the data into depth-wise matrices
```{r}
# Define the properties you want to process
...
# Combine simulated depths w/ simulated properties
```{r}
ssurgo_depth_simulation_data <- ssurgo_depth_simulation@horizons

# Add hzname from ssurgo_data
ssurgo_df <- ssurgo_data@horizons %>% dplyr::select(cokey, hzname, hzdept_r, hzdepb_r)
final_processed_data <- final_processed_data %>% left_join(ssurgo_df, by=c('cokey' = 'cokey', 'hzdept_r'='hzdept_r', 'hzdepb_r'= 'hzdepb_r'))


ssurgo_depth_property_sim <- ssurgo_depth_simulation_data %>% left_join(final_processed_data %>% dplyr::select(unique_id, hzname, bulk_density_third_bar, water_retention_third_bar, water_retention_15_bar, rfv, sand_total,silt_total, clay_total), by = c('id'='unique_id', 'hzname'='hzname'))
...
save(ssurgo_depth_property_sim, final_processed_data, ssurgo_depth_simulation, file = here('data/coweeta-soil-sim-data.rda'))


aws_df <- calculate_aws_df(ssurgo_depth_property_sim@horizons)
...
# BROKEN: from here down, this chunk references simulated_clay, n_depths, and
# n_simulations, none of which are defined anywhere in this file - they were
# copy-pasted from code/coweeta/depthwise-multivariate-profile-simulation.Rmd
# without their setup code and will not run standalone. Flagged, not fixed,
# during the code/ reorganization.
...
gp_pred_means <- predict.GP(gp_model, xnew = as.matrix(depths))$Y_hat
gp_pred_sd <- sqrt(predict.GP(gp_model, xnew = as.matrix(depths))$MSE)
...
plot <- ggplot() +
  ...
  geom_line(aes(x = gp_pred_means, y = depths ), color = 'red', size = 1.5) +
