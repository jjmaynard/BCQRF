...
mukey_sim <- readRDS(here::here('data/mukey_rosetta_simulations.Rds'))
...
VG_sim <- lapply(1:nrow(mukey_sim), function(i) {
  ...
  # BROKEN: KSSL_VG_model() is not defined anywhere in this repo (undeclared
  # external dependency). Flagged, not fixed, during the code/ reorganization.
...
awc_mukey_aws$PIW90 <- awc_mukey_aws$aws95_100 - awc_mukey_aws$aws05_100
## 10m mukey grids (UTM z17), same grid topology
r <- terra::rast(here::here('data/grids/rss_utm.tif'))
...
names(r_stack) <- c("mukey", "aws50_100", "aws05_100", "aws95_100", "PIW90")
...
# local DEM to crop
# UTM
elv <- rast(here::here('data/dss_coweeta/grids/elev_pcs.tif'))

# watershed
w <- vect(here::here('data/vect/Coweeta_Hydrologic_Laboratory.shp'))

# other thematic data for overlays
o <- rast(here::here('data/dss_coweeta/grids/SWI.tif'))
...
plot(r_stack[[3]], col = rev(mako(25)), legend = FALSE, axes = FALSE, maxcell = terra::ncell(r_stack[[3]]), mar = c(0, 0, 0, 0), range=c(0,10))
lines(w, col = 'white')
dev.off()
...
# save as overlay image
png(file = 'overlay-aws_90PIW_revised.png', width = ncol(r_stack[[5]]), height = nrow(r_stack[[5]]))
plot(r_stack[[5]], col = rev(mako(25)), legend = FALSE, axes = FALSE, maxcell = terra::ncell(r_stack[[5]]), mar = c(0, 0, 0, 0), range=c(4,12))
lines(w, col = 'white')
dev.off()

e <- c(275000, 279000, 3877200, 3877500)
png(file = 'legend-aws_revised.png', width = ncol(r_stack[[2]]), height = nrow(r_stack[[2]]))
plot(r_stack[[2]], col = rev(mako(25)), mar = c(7.1, 3.1, 2.1, 3.1), range=c(0,10), plg = list(ext = e, loc = "bottom"))
dev.off()

e <- c(275000, 279000, 3877200, 3877500)
png(file = 'legend-aws_PIWrevised.png', width = ncol(r_stack[[2]]), height = nrow(r_stack[[2]]))
plot(r_stack[[2]], col = rev(mako(25)), mar = c(7.1, 3.1, 2.1, 3.1), range=c(4,12), plg = list(ext = e, loc = "bottom"))
dev.off()

# prepare DEM for rayshader
elmat <- raster_to_matrix(elv)
...
awsPIW90 <- png::readPNG('overlay-aws_90PIW_revised.png')
# compute shadows
raymat <- ray_shade(elmat, multicore = TRUE, progbar = TRUE, zscale = 0.1)
ambmat <- ambient_shade(elmat, multicore = TRUE, progbar = TRUE, zscale = 0.1)
...
elmat %>%
  ...
  add_shadow(raymat) %>%
  add_shadow(ambmat) %>%
  add_overlay(awsPIW90, alphalayer = 0.9) %>%
  plot_3d(elmat, zscale = 10, windowsize = c(px.width, px.height),
          baseshape = 'rectangle', lineantialias = TRUE,
          theta = .theta, phi = .phi, zoom = .zoom, fov = .fov)
...
render_snapshot(
  filename = 'AWS-PIW90-block-diagram.png', clear = TRUE, 
  instant_capture = TRUE
)
...
load(here::here('data/Coweeta_Soil_Simulation.RData'))
```
