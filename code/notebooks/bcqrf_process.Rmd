...
# sim-functions.R was split into code/R/depth_simulation.R and
# code/R/property_simulation.R during the code/ reorganization.
...
```{r}
download_ssurgo_tabular <- function(aoi_wkt, properties= c("db", "cec", "rfv", "clay", "ph", "sand", "silt", "soc")){
  # Define lookup table for general soil properties and their corresponding SoilGrids, SOLUS, and SSURGO labels
  ssurgo_lookup <- data.frame(
    ...
    # SOLUS_Label = c("dbovendry", "cec7", "fragvol", "claytotal", "ph1to1h2o", "sandtotal", "silttotal", "soc"),
  ...
  mukey_list <- terra::values(mu) |> unique() 
  ...
  query <- 'SELECT comp.mukey, comp.cokey, ch.chkey, ch.hzname, ch.hzdept_r, ch.hzdepb_r, ch.sandtotal_r, ch.silttotal_r, ch.claytotal_r, ch.pi_r, ch.lep_r, ch.ec_r, ch.caco3_r, ch.om_r, ch.dbovendry_r, ch.gypsum_r, ch.sar_r, ch.cec7_r, ch.ecec_r, ch.sumbases_r, ch.ph1to1h2o_r, frag.total_fragvol_r, cons.plasticity, cons.stickiness, comp.drainagecl, comp.hydricrating, comp.taxtempcl, comp.frostact, corestr.reskind, corestr.resdept_r AS comp_resdept_r, corestr.reshard, cotax.taxminalogy AS comp_taxminalogy, cm.pondfreqcl, cm.ponddurcl, cm.flodfreqcl, cm.floddurcl, muagg.wtdepannmin FROM CHORIZON ch LEFT JOIN COMPONENT comp ON ch.cokey = comp.cokey LEFT JOIN ( SELECT chkey, SUM(f.fragvol_r) AS total_fragvol_r FROM CHFRAGS f GROUP BY chkey ) AS frag ON ch.chkey = frag.chkey LEFT JOIN CHCONSISTENCE cons ON ch.chkey = cons.chkey LEFT JOIN CORESTRICTIONS corestr ON comp.cokey = corestr.cokey  LEFT JOIN COTAXFMMIN cotax ON comp.cokey = cotax.cokey LEFT JOIN ( SELECT cokey, MAX(monthseq) AS max_month, MAX(pondfreqcl) AS pondfreqcl, MAX(ponddurcl) AS ponddurcl, MAX(flodfreqcl) AS flodfreqcl, MAX(floddurcl) AS floddurcl FROM COMONTH GROUP BY cokey ) AS cm ON comp.cokey = cm.cokey LEFT JOIN MUAGGATT muagg ON comp.mukey = muagg.mukey WHERE comp.mukey IN (2738640,2738668,2738727,1698890,2738708,1698879,1698901,1698880,1698908,1698927,1698909,1698907,1698910,1698928,1698925,1698926,1698897,1698875,1698903,1698929,1698869,1698899,1698891,1698892,1698888,1698887,1698868,2837772,2837878,1698904,1698871,1698874,1698920,1698917,1698906,1698919,1698882,1698872,1698873,1698881,1698893,1698894,2837877,1698916,1698886,1698915,1698885,1698900,1698912) ORDER BY comp.mukey, comp.cokey, ch.hzdept_r;'
  # Submit the query and fetch results using soilDB
  ssurgo_data <- soilDB::SDA_query(query)
  
  if ("rfv" %in% ssurgo_variables) {
...
download_soil_data_sources <- function(aoi_wkt, properties) {
  ...
  ssurgo_comppct_sim <- ssurgo_data |>
    ...
    dplyr::do({
      ...
      sim_component_comp(., n_simulations = 100)
    }) |> dplyr::ungroup()
  
  
  ssurgo_data <- ssurgo_data |> dplyr::left_join(ssurgo_comppct_sim |> dplyr::select(cokey, sim_comppct), by="cokey")
  ...
  ssurgo_data <- ssurgo_data |> as.data.frame() |> dplyr::mutate(id = cokey)
...
properties = "clay"
soil_data <- download_soil_data_sources(aoi_wkt, properties)
```
