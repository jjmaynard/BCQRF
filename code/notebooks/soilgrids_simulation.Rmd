...
# Canonical functions extracted from this notebook during the code/
# reorganization (this file previously redefined these inline in 3 places).
source(here("code/R/distribution_fitting.R"))
source(here("code/R/bayesian_updating.R"))
```
...
# generate_inverse_cdf_distribution() now lives in code/R/distribution_fitting.R
# (extracted during the code/ reorganization as the canonical version of this
# "simulate from percentiles" approach; loaded via the source() call earlier
# in this notebook's setup chunk - see below).
...
# calculate_summary_statistics() now lives in code/R/distribution_fitting.R
# (extracted during the code/ reorganization; loaded via the source() call in
# this notebook's setup chunk).
...
```{r}
# bayesian_update() now lives in code/R/bayesian_updating.R (extracted during
# the code/ reorganization; loaded via the source() call in this notebook's
# setup chunk).
```
...
plot_distributions_and_save <- function(data, distributions_to_plot, prop, highlight_last = TRUE, 
  ...
  # NOTE: previously hardcoded to a different sibling project's folder
  # (C:/R_Drive/.../R_Projects/HWSD.V2/report/figures); redirected to this
  # repo's own reports/figures/ during the code/ reorganization.
  output_file <- here::here("reports", "figures", paste0(prop, paste(distributions_to_plot, collapse = "_"), ".png"))
